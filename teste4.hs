...
-- Função para escrever as incompatibilidades no arquivo
writeIncompatibilidadesToFile :: FilePath -> [(UC, UC, Int)] -> IO ()
writeIncompatibilidadesToFile filePath incompatibilidades =
  withFile filePath WriteMode $ \handle -> do
    hPutStrLn handle "Incompatibilidades entre pares de UCs:"
    mapM_ (\(uc1, uc2, incompat) -> hPutStrLn handle $ "UCs " ++ show uc1 ++ " e " ++ show uc2 ++ ": " ++ show incompat ++ " alunos inscritos em ambas") incompatibilidades
    let totalIncompatibilidades = sum [incompat | (_, _, incompat) <- incompatibilidades]
    hPutStrLn handle $ "Total de incompatibilidades: " ++ show totalIncompatibilidades

-- Exemplo de uso
main :: IO ()
main = do
  let incompatibilidades = calcIncompatibilidades inscricoes
  writeIncompatibilidadesToFile "proposta_escalamento_exames.txt" incompatibilidades
